...
public class FabricantesServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (pathInfo == null || "/".equals(pathInfo)) {
			...
			List<FabricanteDTO> listaFabDTO = listaFabricantes.stream()
			...
			if ("codigo".equals(orden)) {
				if ("asc".equals(modo)) {
					...
					listaFabricantesDTO = fabDAO.getAllDTOPlusCountProductos();
				}
			...
			if (pathParts.length == 2 && "crear".equals(pathParts[1])) {
				...
				try {
					...
					Optional<Fabricante> f = fabDAO.find(id);
					Optional<FabricanteDTO> fabDTO = f.map(fDTO -> new FabricanteDTO(fDTO, fabDAO.getCountProductos(id).orElse(0)));
